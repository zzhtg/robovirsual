...
def frame_ready(image, preview = False):
    """
    输入：image(当前帧图像)
    功能：灰度、二值化、膨胀再腐蚀等处理
    输出：image（处理后图像）
    ...
    ret, image = cv2.threshold(image, down, up, cv2.THRESH_BINARY)
...
def light_detect(frame, color, preview = False):
    """
    ...
    """
    group = []
    pretreatment = frame_ready(frame.img, preview)
